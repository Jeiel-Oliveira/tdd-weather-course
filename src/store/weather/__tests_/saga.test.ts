...
describe('store/weather', () => {
  describe('saga', () => {
    describe('onStart', () => {
      test('Should call fecthWeather API', async () => {
      ...
      test('Should call fecthWeather API and dispatch failure action when unsuccessful', async () => {
        jest.spyOn(WeatherService, 'fetchCurrentWeather').mockRejectedValueOnce(new Error('mock-error'))
        const dispatched = await recordSaga(weatherStartWorker, fetchWeather(0, 0))
